...
from app.routes import recipes, auth, feedback
...
async def health_check():
    ...
    db_exists = os.path.exists(
        os.path.join(os.path.dirname(os.path.dirname(__file__)), settings.DATABASE_PATH)
    )
    return {
        "status": "ok",
        "timestamp": datetime.now().isoformat(),
        "environment": settings.NODE_ENV,
        "database": {"available": db_exists, "type": "SQLite"},
        "smtp": {
